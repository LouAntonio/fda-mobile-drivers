...
export default function DriverEarningsScreen() {
	...
	const payoutMutation = useMutation({
		...
		onSuccess: () => {
			...
			Alert.alert(
				'Sucesso',
				'Saque solicitado com sucesso! O pagamento será processado em dinheiro.',
			);
		},
		onError: (err: any) => {
			const msg =
				...
				'Erro ao solicitar saque. Tenta novamente.';
		...
		<SafeAreaView className="flex-1 bg-off-white dark:bg-[#090909]">
			...
			>
				...
				>
					...
					>
						<Text className="text-xs font-black text-secondary/60 uppercase tracking-widest">
							Saldo Disponível
						</Text>
						...
						<View className="mt-4 pt-4 border-t border-secondary/15">
							<View className="flex-row justify-between">
								<Text className="text-sm font-black text-secondary/60">
									Pendente
								</Text>
				...
				>
					...
					>
						...
						<Text className="text-[10px] font-black text-gray-400 uppercase tracking-wider mt-0.5">
							Disponível
						</Text>
					</View>
					<View
				...
				<Animated.View
					...
					className="mt-8"
				>
					<Text className="text-lg font-black text-secondary dark:text-off-white mb-4">
					...
					{payouts.length === 0 ? (
						<View className="items-center py-12">
							...
							/>
							<Text className="text-base font-black text-secondary dark:text-off-white mt-4">
								Nenhum pagamento ainda
							</Text>
							<Text className="text-sm font-bold text-gray-500 dark:text-gray-400 mt-2 text-center">
								Os pagamentos aparecerão aqui após as viagens
								concluídas.
							</Text>
						</View>
					) : (
						payouts.map((payout, index) => (
							...
							>
								...
								>
									...
									<View
										className={`px-3 py-1.5 rounded-lg ${payout.processedAt ? 'bg-green-500/20' : 'bg-yellow-500/20'}`}
									>
										<Text
											className={`text-xs font-black ${payout.processedAt ? 'text-green-600' : 'text-yellow-600'}`}
										>
											{payout.processedAt
												? 'Pago'
												: 'Pendente'}
						...
						>
							...
							>
								<View className="items-center mb-6">
									...
									>
										O pagamento será processado em dinheiro.
										O valor será descontado do teu saldo
										disponível.
									</Text>
								</View>

								<View className="flex-row items-center bg-gray-100 dark:bg-[#2C2C2E] rounded-2xl px-4 mb-1">
									<Text className="text-lg font-black text-gray-400 mr-2">
										Kz
									</Text>
									<TextInput
