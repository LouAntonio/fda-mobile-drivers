...
export default function HistoryScreen() {
	...
	const getStatusLabel = (status: TripStatus) => {
		const map: Record<string, string> = {
			...
			PICKUP_IN_PROGRESS: 'Em Andamento',
		};
				...
				{isLoading ? (
					<View className="gap-3 pt-2">
						{[1, 2, 3, 4, 5].map((i) => (
							<View
								key={i}
								className="h-28 rounded-2xl bg-gray-200 dark:bg-[#1A1A1A]"
							/>
						))}
					</View>
				) : allTrips.length === 0 ? (
					...
					>
						...
						<Text className="text-sm font-bold text-gray-500 dark:text-gray-400 mt-2">
			...
			>
				...
				>
					...
					>
						...
						>
							<Text
								className={`text-xs font-black uppercase tracking-wider mb-3 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}
							>
								Período
							</Text>
							<View className="flex-row flex-wrap gap-2 mb-6">
								{(
									...
									return (
										...
										>
											<Text
												className={`text-sm font-black ${active ? 'text-secondary' : isDark ? 'text-gray-400' : 'text-gray-500'}`}
											>
												{f === 'all'
													? 'Qualquer'
													: f === 'today'
								...
								>
									<View className="flex-row gap-4">
										<View className="flex-1">
											<Text className="text-xs font-bold mb-1.5 text-gray-500 dark:text-gray-400">
												Início
											</Text>
											<TouchableOpacity
												onPress={() =>
											...
											>
												<Text
													className={`text-sm font-bold ${customStart ? (isDark ? 'text-white' : 'text-black') : 'text-gray-400'}`}
												>
													{customStart
										...
										<View className="flex-1">
											...
											>
												<Text
													className={`text-sm font-bold ${customEnd ? (isDark ? 'text-white' : 'text-black') : 'text-gray-400'}`}
												>
													{customEnd
						...
						<View className="flex-row gap-3 pt-4 border-t border-gray-100 dark:border-gray-900">
							<TouchableOpacity
								className="flex-1 py-3.5 rounded-xl border border-gray-300 dark:border-gray-700 items-center"
								onPress={() => {
									setPeriodFilter('all');
									setCustomStart(null);
									setCustomEnd(null);
							...
							>
								<Text
									className={`text-sm font-black ${isDark ? 'text-gray-300' : 'text-gray-600'}`}
								>
									Limpar
								</Text>
							</TouchableOpacity>
