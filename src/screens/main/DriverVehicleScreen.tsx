	...
	return (
		<SafeAreaView className="flex-1 bg-off-white dark:bg-[#090909]">
			...
			<View className="flex-row items-center justify-between px-5 py-3">
				...
				<Text className="text-lg font-black text-secondary dark:text-off-white">Meu Veículo</Text>
				<TouchableOpacity onPress={() => setShowAddModal(true)} className="w-10 h-10 items-center justify-center rounded-full bg-primary/10">
			...
			<ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false}>
				{isLoading ? (
					<View className="pt-10 gap-4">
						{[1, 2].map((i) => (
							<View key={i} className="h-32 rounded-2xl bg-gray-200 dark:bg-[#1A1A1A]" />
						))}
					</View>
					...
					<Animated.View entering={FadeInDown.duration(600)} className="mt-4">
						{vehicles.map((vehicle, index) => {
							...
							return (
								<Animated.View key={vehicle.id} entering={FadeInRight.duration(500).delay(index * 60)}>
									<View
									...
									>
										<View className="flex-row items-center">
											<View className="w-14 h-14 rounded-2xl items-center justify-center bg-primary/20">
												<Ionicons name={vehicle.type === 'MOTO' ? 'bicycle' : 'car'} size={32} color={themeColors.primary} />
											</View>
											<View className="flex-1 ml-4">
												...
												<View className="flex-row items-center mt-2 gap-2">
													<View className={`px-2.5 py-1 rounded-lg ${isActive ? 'bg-green-500/20' : 'bg-yellow-500/20'}`}>
														<Text className={`text-[10px] font-black ${isActive ? 'text-green-600' : 'text-yellow-600'}`}>
													...
													{isActive && (
														<View className="px-2.5 py-1 rounded-lg bg-primary/20">
															<Text className="text-[10px] font-black text-primary">Atual</Text>
														</View>
													)}
												</View>
											</View>
										...
										{!isActive && vehicle.status === 'ACTIVE' && (
											...
											>
												{setActiveMutation.isPending ? (
													...
													<Text className="text-sm font-black text-primary">Definir como Ativo</Text>
												)}
											</TouchableOpacity>
										)}
										...
										{!isActive && (
											...
											>
												<Text className="text-xs font-black text-red-500">Remover</Text>
											</TouchableOpacity>
										)}
									</View>
								</Animated.View>
					...
					<Animated.View entering={FadeInDown.duration(600)} className="items-center pt-20">
						...
						<Text className="text-lg font-black text-secondary dark:text-off-white">Nenhum veículo cadastrado</Text>
						<Text className="text-sm font-bold text-gray-500 dark:text-gray-400 mt-2 text-center">
							Adicione um veículo para começar
						</Text>
						<TouchableOpacity onPress={() => setShowAddModal(true)} className="mt-6 py-3.5 px-8 rounded-2xl bg-primary active:opacity-70">
				...
				<KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 justify-end">
					...
					<View className={`rounded-t-3xl p-6 pb-10 ${isDark ? 'bg-[#121212]' : 'bg-white'}`}>
						<View className="flex-row items-center justify-between mb-6">
							<Text className={`text-2xl font-black ${isDark ? 'text-white' : 'text-gray-900'}`}>Adicionar Veículo</Text>
							<TouchableOpacity onPress={() => setShowAddModal(false)} className="w-8 h-8 items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-full">
								<Ionicons name="close" size={20} color={isDark ? '#FFF' : '#000'} />
							</TouchableOpacity>
						</View>

						<Text className="text-sm font-black text-gray-500 dark:text-gray-400 mb-3 uppercase tracking-wider">Tipo</Text>
						<View className="flex-row gap-3 mb-5">
							{VEHICLE_TYPES.map((opt) => {
								...
								return (
									...
									>
										<Ionicons name={opt.icon as any} size={18} color={isActive ? '#231F20' : themeColors.text} />
										<Text className={`text-sm font-black ${isActive ? 'text-secondary' : 'text-gray-500 dark:text-gray-400'}`}>{opt.label}</Text>
									</TouchableOpacity>
								);
							})}
						</View>
						...
						<View className="px-4 py-3.5 rounded-2xl border mb-3" style={{ backgroundColor: isDark ? '#2A2A2A' : '#F9FAFB', borderColor: isDark ? '#333' : '#E5E7EB' }}>
							<TextInput className="text-base font-bold" style={{ color: themeColors.text }} placeholder="Placa (ex: LD-45-12-AA)" placeholderTextColor="#9CA3AF" value={plateNumber} onChangeText={setPlateNumber} autoCapitalize="characters" />
						</View>
						<View className="flex-row gap-3 mb-3">
							<View className="flex-1 px-4 py-3.5 rounded-2xl border" style={{ backgroundColor: isDark ? '#2A2A2A' : '#F9FAFB', borderColor: isDark ? '#333' : '#E5E7EB' }}>
								<TextInput className="text-base font-bold" style={{ color: themeColors.text }} placeholder="Marca" placeholderTextColor="#9CA3AF" value={brand} onChangeText={setBrand} />
							</View>
							<View className="flex-1 px-4 py-3.5 rounded-2xl border" style={{ backgroundColor: isDark ? '#2A2A2A' : '#F9FAFB', borderColor: isDark ? '#333' : '#E5E7EB' }}>
								<TextInput className="text-base font-bold" style={{ color: themeColors.text }} placeholder="Modelo" placeholderTextColor="#9CA3AF" value={model} onChangeText={setModel} />
							</View>
						</View>
						<View className="px-4 py-3.5 rounded-2xl border mb-5" style={{ backgroundColor: isDark ? '#2A2A2A' : '#F9FAFB', borderColor: isDark ? '#333' : '#E5E7EB' }}>
							<TextInput className="text-base font-bold" style={{ color: themeColors.text }} placeholder="Cor" placeholderTextColor="#9CA3AF" value={color} onChangeText={setColor} />
						</View>

						<TouchableOpacity onPress={handleAddVehicle} className="py-4 rounded-2xl items-center bg-primary active:opacity-70" disabled={createMutation.isPending}>
